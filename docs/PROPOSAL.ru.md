...
Статус: **v2.2 — approved (2026-09-21). Решения §34 и D1–D3 (§35) закрыты. Фазы 0–7 завершены, следующая — фаза 8 (SSE).**
Дата: 2026-09-21
...
Ещё не отданы (фазы 8 и 9): `events.stream`, `instructions.get` — они перечислены в
`PENDING_ROUTE_IDS`, и тест требует, чтобы этот список совпадал с непокрытыми роутами.
...
**ОТКРЫТО (перед фазой 11):** §15.4 и ADR-0008 говорят «токен встраивается в отдаваемый HTML»,
а бриф фазы 7 требует обратного — «токен не должен попадать в HTML». Фаза 7 токен в HTML не
кладёт (и `APP_CSP` остаётся строгим, без `script-src 'unsafe-inline'`). Как UI получит токен —
решение владельца: (а) inline-инъекция в `index.html` (тогда CSP нужен nonce), (б) отдельный
`GET /api/v1/session`, читаемый только same-origin (Origin-проверка + отсутствие CORS-заголовков),
(в) что-то третье. До решения UI мутировать не может.
