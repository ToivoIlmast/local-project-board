...
Статус: **v2.2 — approved (2026-09-21). Решения §34 и D1–D3 (§35) закрыты. Фаза 0 завершена, идёт фаза 1.**
Дата: 2026-09-21
...
interface Task {
  ...
  updatedAt: string;                     // ISO; optimistic concurrency
...
битый frontmatter → карточка `broken`, не падение (**ИНВАРИАНТ**).
...
**Conformance suite** (`test/conformance/storage.ts`) — параметризован фабрикой, гоняется по
...
```text
Task
├── структурированные метаданные → Storage (сейчас: task.md frontmatter)
└── документы                   → файлы .board/tasks/<id>/<name>
Reports                          → файлы .board/reports/<id>.{html,md}
```

При любом будущем провайдере метаданных документы остаются файлами. Имя документа:
`^[A-Za-z0-9._-]{1,64}\.(md|html)$`. Standalone-снимок (§23) содержит *копии* документов;
...
- user-level — порт, открывать ли браузер, команда редактора, секреты будущих провайдеров
  ...
  `.board/runtime.json`.
- zod `.strict()`; **ИНВАРИАНТ:** невалидный конфиг → человекочитаемая ошибка с путём к ключу
  и ненулевой exit code, не стектрейс.
- удалён статус, в котором есть задачи → сервер не стартует молча, сообщает число осиротевших задач.
...
- Реализация MVP: zod-схема в `core/model`, чистая функция `createSnapshot(storage)` в ядре,
  CLI `board export [--out <file>]`. Поле `formatVersion: 1` — одна строка, без механизма миграций.
