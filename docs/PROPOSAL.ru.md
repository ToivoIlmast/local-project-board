...
```text
GET    /api/v1/project
GET    /api/v1/session                      ← токен этого запуска для собственной страницы доски
...
Канонический список — `src/contract/v1/routes.ts` (22 роута); список выше — его читаемая форма.

Как это устроено в коде (фаза 7): роутер **строится из таблицы роутов**, а не пишется руками, —
роут не может появиться без контракта, а контракт не может потерять роут (`http/v1/router.ts`).
Валидация — теми же схемами (`parseParams` / `parseQuery` / `parseBody`), сериализация — по
`response.media`, статус успеха — по `successStatus` (201 у двух create-роутов, иначе 200).
Хендлеры (`http/v1/handlers.ts`) делают только «взять провалидированное → позвать сервис →
вернуть ответ»; ранжирование, статусы и права остаются в `core`. Формат ошибки —
`errorResponseSchema` в контракте: коды домена берутся из `BOARD_ERROR_CODES` ядра, транспортные —
из `TRANSPORT_ERROR_CODES`, так что список кодов один и разойтись ему негде.
Дополнительно к §13: 405 + `Allow` на чужой метод известного пути, 413 на тело больше 2 МБ,
`Cache-Control: no-store` на всех ответах API, `/api/*` вне `/api/v1` — всегда JSON-404, никогда SPA.
Фаза 9 закрыла последние два роута: `instructions.get` и добавленный в контракт `session.get`.
Теперь хендлер есть у каждого роута, и это гарантирует тип: `Handlers` — это
`{ [K in Exclude<RouteId, 'events.stream'>]: RouteHandler<K> }`, поэтому новый роут в контракте
не компилируется, пока его не обслужили. Единственное исключение — `events.stream`: поток не
ответ, роутер отдаёт его SSE-сериализатору (§14). Списка `PENDING_ROUTE_IDS` больше нет.
Генератор AI Instructions сделан в фазе 2, а не в фазе 9: он — часть того же источника правды,
и drift-тест без него не имеет смысла. Фаза 9 добавила к нему живые факты доски и эндпоинт.
`instructions.get` и `session.get` получают то, что знает только запущенный сервер, через
`RouteContext.session = { baseUrl, token }`; сервисы о существовании токена не знают.
...
Как это сделано (фаза 8):

- `server/events/` — in-process шина: `publish` (это и есть порт `EventSink`) + `subscribe`,
  возвращающий отписку, и `size` — сколько слушателей сейчас. Публикация идёт по **копии**
  набора слушателей, а ошибка слушателя ловится и уходит в `onListenerError`:
  **ИНВАРИАНТ** — мёртвая вкладка браузера не может уронить запись на доску;
- `http/sse.ts` — сериализатор: `retry:` один раз в начале, `data: <json>\n\n` на событие,
  комментарий `:ping` как heartbeat (таймер `unref`, чтобы не держать процесс). На `close`
  запроса или ответа — `clearInterval` + отписка: **ИНВАРИАНТ** — ушедший клиент не остаётся
  в шине (проверяется через `bus.size`);
- журнала событий нет: клиент, который подключился, видит то, что происходит дальше, а после
  реконнекта перезапрашивает всё (§14 выше). Поэтому нет ни `Last-Event-ID`, ни `id:`;
- завершение сервера — `closeServer()` (`close` + `closeAllConnections`): открытый поток иначе
  держал бы `server.close()` ровно столько, сколько открыта вкладка. **ИНВАРИАНТ** — тест на то,
  что сервер останавливается при открытом потоке;
- **РЕШЕНИЕ владельца (2026-09-23):** поток остаётся read-only и **без Bearer-токена** —
  стандартный `EventSource` не умеет слать заголовки. Защищают Host- и Origin-проверки: чужая
  страница получает 403 и потока не видит. Переход на fetch-streaming ради авторизации сейчас
  не делается; если появятся чувствительные события — пересматривается отдельно.
...
7. HTML-отчёты — `<iframe sandbox>` (без `allow-same-origin`) + CSP на ответе
   `/reports/:id` и на `.html`-документах (ADR-0023); markdown рендерится через `rehype-sanitize`.
...
Как это сделано (фаза 7):

- проверка Host/Origin стоит **перед всем**, включая саму страницу приложения: rebinding не
  должен получать даже HTML. Host обязан быть `localhost`/`127.0.0.1` **с портом этого сервера**;
  `[::1]` не принимается (сервер слушает IPv4-loopback), отсутствие Host — тоже отказ;
- Origin либо отсутствует (curl, агент), либо `http://{localhost|127.0.0.1}:<тот же порт>`;
  `Origin: null` (sandbox-фрейм, `file://`) — это **не** наш origin, отказ;
- токен: `Authorization: Bearer <token>` (схема регистронезависима, значение — нет),
  сравнение `timingSafeEqual` по байтам, 256 бит из системного CSPRNG на каждый запуск;
  из URL токен не берётся никогда; мутации без него — 401, чтение — без токена;
- CSP три штуки: `API_CSP` (`default-src 'none'`, не фреймится), `APP_CSP` (только своё,
  `script-src 'self'`, `frame-ancestors 'none'`, `frame-src 'self'` — чтобы вставлять отчёты),
  `EMBEDDED_HTML_CSP` на HTML-отчёты и `.html`-документы: `sandbox allow-scripts` **без**
  `allow-same-origin` (свой origin, ни DOM доски, ни её токена) плюс `connect-src 'none'`
  и `img/font-src data:` — отчёт остаётся интерактивным, но офлайн;
- 500 не содержит ни сообщения, ни стека, ни путей; причина уходит в лог сервера.

**РЕШЕНИЕ владельца (2026-09-23), заменяет §15.4 в части доставки токена:** токен **не**
встраивается в HTML. UI получает его обычным same-origin `fetch` через `GET /api/v1/session`
(роут добавлен в контракт в фазе 9; на него действуют те же Host/Origin-проверки,
что и на остальные локальные эндпоинты; ответ — ровно `{ "token": "..." }` и ничего больше). `APP_CSP` остаётся строгим: `script-src 'self'`, без
inline и без nonce, пока nonce не нужен. Токен не логируется и не попадает в URL.
`GET /api/v1/instructions` токена не требует (read-only, и сценарий «человек или агент быстро
берёт инструкции» важнее): чужая страница ответ не прочитает — Origin отклоняется и CORS-заголовков
нет. То, что инструкции содержат токен для мутаций, — задумка инструмента: пользователь сам решает,
кому их отдать.
...
**ИНВАРИАНТЫ (тесты, `test/api/security.test.ts`):** чужой Origin → 403 (и на чтении тоже);
чужой Host, чужой порт, отсутствующий Host → 403; мутация без токена, с чужим токеном, с токеном
в URL → 401; токен возвращают ровно два роута — `/session` и `/instructions` (проверка идёт по
всем GET-роутам контракта, поэтому новый роут не проскочит мимо неё), и ни один не кладёт его
в заголовок; токен не попадает в вывод сервера (console перехвачен на время теста); `../`, `..%2f`, `%2e%2e%2f`, двойное кодирование, абсолютный путь в имени документа →
400/404 и никогда содержимое файла вне доски; ответ `/reports/:id` и `.html`-документа содержит
`sandbox` без `allow-same-origin`; токен не встречается ни в одном ответе; страница приложения
отдаётся только со своим Host; тело JSON-роута парсится только как JSON (`text/plain` с верным
токеном и без Origin — 400, а не задача); имя хоста сравнивается без учёта регистра.
...
Что в тексте (фаза 9): назначение доски; живой base URL `http://127.0.0.1:<port>/api/v1`;
аутентификация (`Authorization: Bearer <token>`, чтение — без токена, токен только в заголовке);
живые факты доски — имя проекта, статусы, idPrefix (берутся из `project.read()` на каждый запрос,
поэтому агенту не нужно угадывать статус); правила (move по соседям, имена документов, sandbox
для HTML-отчётов, лимиты); по каждому роуту — метод, путь, конкретный пример пути и query,
пример тела, статус успеха, media и пример ответа; формат ошибки и полный список кодов из
`errorCodeSchema`. Чего в тексте нет: внутренней архитектуры (Express, React, storage, классы) —
агенту нужна API surface. Playbook'и audit/plan/review и `allowSourceEdits` остаются на будущее:
сейчас правила — это то, что нарушает доску, а не то, как вести проект.

**ИНВАРИАНТ (drift-тест):** множество заголовков `### METHOD /api/v1/...` в тексте **равно**
множеству роутов с `ai.include: true` — ни одного лишнего, ни одного пропущенного; `ai.include` —
единственный фильтр (новый ресурс получает свой раздел, а не исчезает); каждый JSON-блок в тексте
либо пример роута, либо форма ошибки, и валиден по схеме контракта.

**Эндпоинт не документирует сам себя:** `instructions.get` — `ai.include: false`, иначе инструкция
ссылалась бы на себя. `events.stream` — тоже `false`: агент опрашивает то, что изменил.
