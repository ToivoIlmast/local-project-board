...
Статус: **v2.2 — approved (2026-09-21). Решения §34 и D1–D3 (§35) закрыты. Фазы 0–8 завершены, следующая — фаза 9 (эндпоинт инструкций).**
Дата: 2026-09-21
...
Ещё не отдан (фаза 9): `instructions.get` — он перечислен в `PENDING_ROUTE_IDS`, и тест требует,
чтобы этот список совпадал с непокрытыми роутами. `events.stream` хендлера тоже не имеет, но по
другой причине: поток — не ответ, роутер отдаёт его SSE-сериализатору (§14).
...
(роут добавляется в контракт вместе с UI-фазой; на него действуют те же Host/Origin-проверки,
что и на остальные локальные эндпоинты). `APP_CSP` остаётся строгим: `script-src 'self'`, без
...
в URL → 401; `../`, `..%2f`, `%2e%2e%2f`, двойное кодирование, абсолютный путь в имени документа →
